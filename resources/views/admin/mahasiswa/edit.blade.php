                    ...
                    <form action="{{ route('admin.mahasiswa.update', $mahasiswa) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $mahasiswa->email)" required />
                                <x-input-error :messages="$errors->get('email')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="no_hp" :value="__('No. HP')" />
                                <x-text-input id="no_hp" class="block mt-1 w-full" type="text" name="no_hp" :value="old('no_hp', $mahasiswa->no_hp)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="angkatan" class="block mt-1 w-full" type="number" name="angkatan" :value="old('angkatan', $mahasiswa->angkatan)" />
                            ...
                            <div>
                                ...
                                <select id="role" name="role" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500">
                                    <option value="anggota" {{ old('role', $mahasiswa->role) === 'anggota' ? 'selected' : '' }}>Anggota</option>
                                    <option value="admin" {{ old('role', $mahasiswa->role) === 'admin' ? 'selected' : '' }} {{ !auth()->user()->isAdministrator() ? 'disabled' : '' }}>Admin</option>
                                    <option value="administrator" {{ old('role', $mahasiswa->role) === 'administrator' ? 'selected' : '' }} {{ !auth()->user()->isAdministrator() ? 'disabled' : '' }}>Administrator</option>
                                </select>
